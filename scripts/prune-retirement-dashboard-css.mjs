...
function pruneCss(css, authoritativeSelectors, stats) {
  ...
  for (const block of blocks) {
    ...
    if (block.header.startsWith("@")) {
      ...
      const isFullyOwned =
        selectors.length > 0 &&
        selectors.every((selector) => authoritativeSelectors.has(selector));
...
const original = await readFile(dashboardCssPath, "utf8");
const stats = {
  rulesRemoved: 0,
  selectorsRemoved: 0,
  emptyAtRules: 0,
...
console.log(`retirement-dashboard.css rules removable: ${stats.rulesRemoved}`);
console.log(`retirement-dashboard.css selectors removable: ${stats.selectorsRemoved}`);
...
if (shouldWrite) {
  await writeFile(dashboardCssPath, `${pruned.trimEnd()}\n`, "utf8");
  console.log("\nretirement-dashboard.css updated.");
} else {
  console.log(
    "\nDry run only. Re-run with --write to update retirement-dashboard.css."
  );
}
