  ...
  return (
    ...
    >
      ...
      <div className="scenario-edit-grid">
        <div className="form-field">
          ...
          <div className="income-target-toggle" role="group" aria-label="Retirement household">
            <button type="button" className={household === "one-person" ? "income-target-option income-target-option-active" : "income-target-option"} aria-pressed={household === "one-person"} onClick={() => updateHousehold("one-person")}>One person</button>
            <button type="button" className={household === "two-person" ? "income-target-option income-target-option-active" : "income-target-option"} aria-pressed={household === "two-person"} onClick={() => updateHousehold("two-person")}>Two people</button>
          </div>
        </div>
        ...
        <div className="form-field">
          ...
          <div className="income-target-toggle" role="group" aria-label="Retirement area">
            <button type="button" className={region === "uk" ? "income-target-option income-target-option-active" : "income-target-option"} aria-pressed={region === "uk"} onClick={() => updateRegion("uk")}>UK</button>
            <button type="button" className={region === "london" ? "income-target-option income-target-option-active" : "income-target-option"} aria-pressed={region === "london"} onClick={() => updateRegion("london")}>London</button>
          </div>
        </div>
      </div>

      <div className="drawdown-lifestyle-progression" aria-label="Retirement spending progression">
        <ProgressionCard label="Your target spending" value={formatCurrency(progression.targetSpending)} detail="Estimated net spending from the current retirement target." />
        <ProgressionCard label="Sustainable spending" value={formatCurrency(progression.sustainableSpending)} detail={`Illustrated annual spending supported to age ${inputs.endAge} under the selected ending-balance goal.`} />
        <ProgressionCard label="Living Standard supported" value={progression.supportedLevel ? levelLabel(progression.supportedLevel) : "Below Minimum"} detail={progression.supportedAmount === null ? `The Minimum benchmark is ${formatCurrency(standards.minimum)}.` : `${formatCurrency(progression.supportedAmount)} annual benchmark.`} />
      </div>

      <div className="drawdown-lifestyle-story">
        <div>
          <span className="drawdown-decision-label">Your target</span>
          <strong>{progression.targetHeadroom >= 0 ? `${formatCurrency(progression.targetHeadroom)} annual headroom` : `${formatCurrency(Math.abs(progression.targetHeadroom))} annual shortfall`}</strong>
          <p>{progression.targetHeadroom >= 0 ? `The illustration supports your current ${formatCurrency(progression.targetSpending)} net spending target to age ${inputs.endAge}.` : `Your current ${formatCurrency(progression.targetSpending)} net spending target is above the illustrated sustainable level.`}</p>
        </div>

        <div>
          <span className="drawdown-decision-label">Lifestyle context</span>
          <strong>{progression.supportedLevel ? `${levelLabel(progression.supportedLevel)} lifestyle supported` : "Minimum benchmark not yet supported"}</strong>
      ...
      <p className="scenario-tax-free-cash-note">
        ...
        <a href={RETIREMENT_LIVING_STANDARDS_2026.sourceUrl} target="_blank" rel="noreferrer">{RETIREMENT_LIVING_STANDARDS_2026.sourceName}</a>.
      </p>
    </section>
  );
}

function ProgressionCard({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
...
function Benchmark({ level, amount, sustainableIncome }: { level: string; amount: number; sustainableIncome: number }) {
  ...
  return (
    <article className="drawdown-benchmark-card">
      ...
      <small>{reached ? `${formatCurrency(difference)} annual headroom` : `${formatCurrency(Math.abs(difference))} below benchmark`}</small>
    </article>
  );
}
